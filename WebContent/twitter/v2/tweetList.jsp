...
<!-- 기본 URL 경로 지정 -->
<c:set var="baseUrl" value="${pageContext.request.contextPath}/twitter/v2" />
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
	...
	<div class="container shadow mx-auto mt-5 p-5 w-60">
		...
		<form action="${baseUrl}" method="post">
			...
			<div class="input-group">
				<button type="button" class="btn btn-outline-success">@${user.name}</button>
		...
		<div align="left">
			<ul class="list-group">
				<c:forEach var="tweet" items="${tweetlist}">
							<%-- 수정 --%>
							<li class="list-group-item list-group-item-action">[${tweet.id}]${tweet.name}]:: ${tweet.msg}, ${tweet.date}
							<a href="${baseUrl}?action=del&id=${tweet.id}"><button type="button" class="close">&times;</button></a>
							</li>
				</c:forEach>
			</ul>
		</div>
